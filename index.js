        ...
        if (message.member.permissions.has(config.perms)) {
            ...
            if (!args[0]) {
                ...
                let row = new MessageActionRow()
                    .addComponents(
                        new MessageButton()
                        .setLabel(`กด React เพื่อรับ: ยศ ${role.name}`)
                        .setStyle("PRIMARY")
            ...
            message.channel.send({ content: "คุณไม่ได้รับอนุญาต" })
        }
    }
})
...
VS.on('interactionCreate', async(interaction) => {
    ...
    if (interaction.member.roles.cache.has(role.id)) {
        ...
        let e1 = new MessageEmbed()
            .setColor(config.embed.color)
            .setDescription(`ยศ ${role.name} ได้ถูกลบออกจากคุณแล้ว!`)
        interaction.followUp({ embeds: [e1], ephemeral: true })
    } else {
        interaction.member.roles.add(role.id)
        let e1 = new MessageEmbed()
            .setColor(config.embed.color)
            .setDescription(`คุณได้รับยศ ${role.name} เรียบร้อยแล้ว!`)
        interaction.followUp({ embeds: [e1], ephemeral: true })
    }
...
VS.login(config.token).catch(e => {
    if (e) {
        console.log(` You have an invalid token/No token added  in enmap`)
    }
})
